  ...
  if (data && data.length > 0) {
    ...
    if (matched.deleted_at) {
      if (!editingId.value) {
        ...
        errors.nombre = `Esta marca ya existe pero fue eliminada: "${matched.nombre}".`
      } else {
        errors.nombre = `Esta marca ya existe como eliminada.`
      }
    } else {
      errors.nombre = `Esta marca ya existe y está activa.`
    }
  }
}
