...
<template>
  <div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Categorías de repuestos</h1>
    ...
    <form class="bg-white p-5 rounded-lg shadow-sm mb-6 flex flex-col sm:flex-row gap-4 sm:items-end" @submit.prevent="submit">
      <div class="flex-1">
        <label class="block text-sm font-medium text-slate-600 mb-1">
          {{ editingId ? 'Editar Nombre de la Categoría' : 'Nombre de la Categoría' }}
        </label>
        <input 
          v-model="form.nombre" 
          required 
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-x-auto border border-slate-100">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-if="loading">
            <td class="px-5 py-4 text-slate-500" colspan="2">Cargando categorías...</td>
          </tr>
          <tr v-else-if="categorias.length === 0">
            <td class="px-5 py-4 text-slate-500 text-center" colspan="2">No hay categorías registradas aún.</td>
          </tr>
  ...
  >
    ...
    >
      ...
      <p class="text-sm text-slate-500 mb-6">
        Esta acción marcará la categoría como eliminada. Los repuestos asociados a esta categoría seguirán existiendo en el histórico de movimientos.
      </p>
  ...
  >
    ...
    >
      ...
      <p class="text-sm text-slate-500 mb-4">
        Ya existe una categoría registrada con este nombre que fue eliminada anteriormente. ¿Deseas reactivarla?
      </p>
      
      <div class="bg-slate-50 p-4 rounded-2xl mb-6 text-xs space-y-2 border border-slate-100">
        <p class="text-slate-500 font-semibold uppercase tracking-wider text-[10px]">Datos de la categoría guardada:</p>
